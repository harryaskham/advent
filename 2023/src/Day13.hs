...
reflects :: STVectorGrid s Char -> ST s (Set (Either ℤ' ℤ'))
reflects g = do
  (maxX, maxY) <- maxXYM g
...
reflectsSmudged :: STVectorGrid s Char -> ST s (Set (Either ℤ' ℤ'))
...
solve :: (STVectorGrid s Char -> ST s (Set (Either ℤ' ℤ'))) -> ST s [STVectorGrid s Char] -> ST s ℤ'
